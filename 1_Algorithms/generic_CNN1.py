...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D, BatchNormalization, Activation,AveragePooling2D
...
x = Activation('relu')(x)
x = AveragePooling2D((poolSize, poolSize), padding='same')(x)
#x = MaxPooling2D((poolSize, poolSize), padding='same')(x)
x = Conv2D(numKernels, (5, 5), padding='same')(x)
x = BatchNormalization()(x)
x = Activation('relu')(x)
encoded = AveragePooling2D((poolSize, poolSize), padding='same')(x)
