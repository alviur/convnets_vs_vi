...
chromosomes = rand(6,numBits,population*2);
...
imageSet = makeGridImage2(chromosomes);
...
bestParameters =  zeros(6,numBits,generations*2);
...
%         imageSet(:,:,1,i+100) = (imagesOld(:,:,1,reproductionIndices(i)+100).*Mask + imagesOld(:,:,1,i+100)).*not(Mask);
    ...
    for i=1:int8(mutationRate*population)
       ...
       chromosomes(feature,bitt,index+population) = not(chromosomes(feature,bitt,index+population));
%        
...
%     pause
    ...
    difference = abs(abs(light) - abs(dark));
